...
async def schedule_scratch(client, chat_info):
    ...
    # пост с карточками про Scratch во второй день
    await client.send_file(
        chat_info["id"],
        imgs["cards"],
    ...
    # приглашение на 3 урок
    await client.send_file(
        chat_info["id"],
        imgs["invitation"],
        caption=messages["invitation"].format(
            hour=dates["course_hour"], minute=dates["course_minute"]
        ),
        schedule=dates["feedback_2"],
    )

    # напоминание о третьем уроке
